...
def make_SIMULATION_utilisation_variation_plot(
    ...
    utilisation_df_per_run['callsign'] = utilisation_df_per_run['callsign'].astype(str)
    ...
    fig.update_yaxes(categoryorder='array', categoryarray=sorted_callsigns, autorange="reversed")

    # Add historical data
    if historical_utilisation_df_summary is not None:
        historical_utilisation_df_summary.index = historical_utilisation_df_summary.index.astype(str)
        ...
        for num_idx, callsign_str in enumerate(sorted_callsigns):
            if callsign_str in historical_utilisation_df_summary.index:
                ...
                fig.add_shape(
                    type="rect",
                    yref="y", xref="x",
                    y0=y_pos_start, x0=min_val,
                    y1=y_pos_end, x1=max_val,
                ...
                fig.add_shape(
                    type="line",
                    yref="y", xref="x",
                    y0=y_pos_start, x0=mean_val,
                    y1=y_pos_end, x1=mean_val,
                    line=dict(
                        dash="dot",
